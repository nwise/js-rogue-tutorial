...
export function renderHealthBar(
  ...
  drawColoredBar(display, 0, 29, totalWidth, Colors.BarEmpty);
  drawColoredBar(display, 0, 29, barWidth, Colors.BarFilled);

  const healthText = `HP: ${currentValue}/${maxValue}`;

  for (let i = 0; i < healthText.length; i++) {
    display.drawOver(i + 1, 29, healthText[i], Colors.White, null);
  }
}
